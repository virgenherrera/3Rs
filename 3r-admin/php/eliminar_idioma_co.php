...
//en esta seccion se hace el proceso de eliminar lenguaje a la seccion CONTACTO
if(isset($_POST['idioma'])	&& !empty($_POST['idioma'])){
	
	//eliminar registro del producto de la bdd
	$consulta = 'DELETE FROM contacto WHERE idioma = "' . $_POST['idioma'] . '"';
	insertar_mysql($consulta);
	
	//redirigir al finalizar
	header('location: ../panel-de-control.php?msg=elim_lang_co');
}
//en esta seccion se hace el proceso de redirigir si no se puedo validar ninguna adicion
	...
	header('location: ../panel-de-control.php?msg=er_lang_co');	
}
?>